...
def osemosys_model(file_dat, file_json, solver):
    ...
    model.DGTagTechnologyFuel = Param(
        model.REGION,
        model.FUEL,
        model.TECHNOLOGY,
        default=0
    )
    model.DGTimeSliceLowerLimitRatio = Param(
        model.REGION,
        model.FUEL,
        model.YEAR,
        model.TIMESLICE,
    ...
    model.ProductionByDGTechnology = Var(
        model.REGION,
        model.TIMESLICE,
        model.TECHNOLOGY,
        model.FUEL,
    ...
    model.MinProductionByDGTechnology = Var(
        model.REGION,
        model.TIMESLICE,
        model.FUEL,
        model.YEAR,
    ...
    def DGTechnologyProductionTimeSlice_rule(model, r, l, t, f, y):
        return (
            model.DGTagTechnologyFuel[r, f, t] * model.ProductionByTechnology[r, l, t, f, y]
            == model.ProductionByDGTechnology[r, l, t, f, y]
        )
    
    model.DGTechnologyProductionTimeSlice = Constraint(
        model.REGION, model.TIMESLICE, model.TECHNOLOGY, model.FUEL, model.YEAR, rule=DGTechnologyProductionTimeSlice_rule
    )

    def MinProductionByDGTechnologyTimeSlice_rule(model, r, l, f, y):
        return (
            model.DGTimeSliceLowerLimitRatio[r, f, y, l] * model.Production[r, l, f, y]
            == model.MinProductionByDGTechnology[r, l, f, y]
        )
    
    model.MinProductionByDGTechnologyTimeSlice = Constraint(
        model.REGION, model.TIMESLICE, model.FUEL, model.YEAR, rule=MinProductionByDGTechnologyTimeSlice_rule    
    )
    
    def DGTimeSliceBalance_rule(model, r, l, f, t, y):
        return(
            sum(
            model.ProductionByDGTechnology[r, l, t, f, y]
            for t in model.TECHNOLOGY
    ...
    model.DGTimeSliceBalance = Constraint(
        model.REGION, model.TIMESLICE, model.FUEL, model.TECHNOLOGY, model.YEAR, rule=DGTimeSliceBalance_rule    
    )


    ## SOLVING THE MODEL
    instance = model.create_instance(
        file_dat)  # creating the concrete model from the abstract model and the input data file
    opt = SolverFactory(solver)  # selecting the solver
    results = opt.solve(instance, load_solutions=True, tee=True)  # solving the problem
    instance.solutions.store_to(results)  # copying the numerical results
